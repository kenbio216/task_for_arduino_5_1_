...
LiquidCrystal_I2C lcd(0x27,16,2);     //设备地址，显示字符数、行数
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  Serial.println();
  snprintf (distance, length+1, "%s", payload);
...
void setup() {
  ...
  lcd.init();                         // LCD初始化液晶屏
  lcd.backlight();                    //LCD打开背光
  lcd.clear();                        //清屏
  lcd.setCursor(0,0);                 //调整光标到第1行的第3列
...
//  LCD页面显示
 ...
 lcd.clear();                        //清屏
  ...
  lcd.setCursor(0,1);                 //调整光标到第2行的第0列
  lcd.print("zs2201 tz lzd"); //显示班级和姓名
  delay(1000);
...
//发布亮度传感器数值
  ...
  if (now - lastMsg > 2000) {
    lastMsg = now;
    snprintf (msg, 50, "%d", analogRead(A0));//如果已经过了2秒，更新lastMsg，value值加1。准备将value值发布到MQTT服务器，像上面的KEY值一样处理成字符串
    Serial.print("童卓 2022030233 罗子丹 2022020584 亮度传感器数值: ");
    Serial.println(msg);
    client.publish("ledgan", msg);//ESP8266开发板PUBLISH一个主题为temp内容为msg的消息
  }
}